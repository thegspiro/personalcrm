...
test("they remain on the overdue list", async ({ page }) => {
  ...
  // actionable filter must retain the due person and leave out a contact with
  // no cadence, even though both would appear in an overdue-sorted full list.
  await page.goto("/");
  ...
  await expect(page).toHaveURL(/due=actionable/);
