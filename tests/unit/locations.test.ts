...
describe("mapLinkFor", () => {
  it("prefers the OSM object, which survives a reimport", () => {
    expect(mapLinkFor({ osmType: "W", osmId: 12345n, name: "Northside Cafe" })).toBe(
  ...
  it("marks a point rather than searching for the digits", () => {
    ...
    expect(mapLinkFor({ latitude: "38.8809", longitude: "-77.1728", name: "Home" })).toBe(
  ...
  it("ignores half a coordinate pair and unparseable values", () => {
    expect(mapLinkFor({ latitude: "38.88", longitude: null, name: "Half" })).toBe(
      "https://www.openstreetmap.org/search?query=Half",
    );
    expect(mapLinkFor({ latitude: "north", longitude: "west", name: "Vague" })).toBe(
      "https://www.openstreetmap.org/search?query=Vague",
    );
  });
});
