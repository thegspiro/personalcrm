...
describe("backoffRemainingMs", () => {
  it("lets the first attempts through at full speed", () => {
    for (let count = 0; count < FAILURES_BEFORE_BACKOFF; count += 1) {
      expect(backoffRemainingMs(count, new Date(T0), T0)).toBe(0);
    }
  });

  it("waits five seconds once the allowance is spent", () => {
    expect(backoffRemainingMs(FAILURES_BEFORE_BACKOFF, new Date(T0), T0)).toBe(5_000);
  });

  it("doubles with each further attempt", () => {
    expect(backoffRemainingMs(6, new Date(T0), T0)).toBe(10_000);
    expect(backoffRemainingMs(7, new Date(T0), T0)).toBe(20_000);
    expect(backoffRemainingMs(8, new Date(T0), T0)).toBe(40_000);
  });
  ...
  it("never asks anyone to wait more than the ceiling", () => {
    ...
    expect(backoffRemainingMs(100, new Date(T0), T0)).toBe(MAX_BACKOFF_MS);
  });

  it("counts down as real time passes", () => {
    expect(backoffRemainingMs(6, new Date(T0), T0 + 4_000)).toBe(6_000);
    expect(backoffRemainingMs(6, new Date(T0), T0 + 10_000)).toBe(0);
    // Never negative: a long-past failure is simply over, not owed back.
    expect(backoffRemainingMs(6, new Date(T0), T0 + 999_000)).toBe(0);
  });
...
describe("isStale", () => {
  it("treats a fresh attempt as current", () => {
    expect(isStale(new Date(T0), T0 + 1_000)).toBe(false);
    expect(isStale(new Date(T0), T0 + ATTEMPT_TTL_MS)).toBe(false);
  });

  it("forgets attempts past the retention window", () => {
    // Five typos last week must not throttle the first attempt today.
    expect(isStale(new Date(T0), T0 + ATTEMPT_TTL_MS + 1)).toBe(true);
  });
