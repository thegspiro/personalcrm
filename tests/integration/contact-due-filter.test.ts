...
describe.skipIf(!hasTestDatabase)("contact due-status filter", () => {
  ...
  it("includes contacts due on the account's current day and excludes later and no-cadence contacts", async () => {
    ...
    try {
      ...
      const [actionable, soon] = await Promise.all([
        listContacts(ownerId, { dueStatus: "actionable", sort: "overdue" }, "Pacific/Honolulu"),
        listContacts(ownerId, { dueStatus: "soon", sort: "overdue" }, "Pacific/Honolulu"),
      ]);

      expect(actionable.items.map((contact) => contact.firstName)).toEqual([
        "Due today",
      ]);
      expect(actionable.total).toBe(1);

      // "Due soon" is what the dashboard widget links to, so it has to reach
      // the days the widget shows — while still leaving out no-cadence rows.
      expect(soon.items.map((contact) => contact.firstName)).toEqual([
        "Due today",
        "Due later",
      ]);
      expect(soon.total).toBe(2);
    } finally {
      vi.useRealTimers();
    }
  });
