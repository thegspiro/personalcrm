...
| Path | Why it matters |
| --- | --- |
| `db/` | The live MariaDB data directory. Copy it only while the container is stopped. |
| `uploads/` | Avatars and photos (reserved; uploads are not implemented yet). |
| `secrets.json` | Session signing/encryption material and the bundled database password. |
| `backups/` | Automatic compressed SQL dumps. |
| `logs/`, `cache/` | Disposable runtime data. |
...
This captures the live database directory, uploads, secrets, and automatic SQL
dumps together. Never filesystem-copy `/config/db` while MariaDB is running.
