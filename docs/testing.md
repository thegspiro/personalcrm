...
- `reminders.test.ts` — all four delivery policies; that a cadence due later
  ...
  cancellation after task, date or privacy changes; that a digest projects
  timezone-local important dates, cadences, and tasks across three dated days,
  preserves partial-date recurrence policy and privacy/owner boundaries, and
  is retried within its day with a fresh complete projection without sending
  look-ahead reminders early; that it waits for an hour moved later and is
