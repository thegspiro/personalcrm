...
| **Sign-in throttling trusts the forwarded address** | Repeated wrong passwords back off per address-and-client pair, but the client half is whatever the request presents as `X-Forwarded-For` and nothing verifies it. It stops one client grinding a password list; it does not stop one that varies the header. See [privacy.md](privacy.md#sign-in-throttling) |
