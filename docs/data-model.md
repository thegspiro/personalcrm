...
| `status` | `PlanStatus` | `OPEN` \| `PLANNED` \| `DONE` \| `ARCHIVED`. `listPlans` reads open rows by default and closed ones through `closedOnly`, which is a separate view rather than a widened list: the enum sorts open-first and the query is capped, so an account whose open plans fill the page would never reach a closed one. That view also orders by `updatedAt` rather than by status, for the same reason one level down — DONE sorts before ARCHIVED, so a status-first ordering would truncate the archived half away. A closed plan is read-only: `updatePlan` refuses it until `setPlanStatus` puts it back on the list |
