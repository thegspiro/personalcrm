...
| `mergeDuplicate` | Folds one contact into another, permanently. Lock-gated. Takes a **side** per field, never a value — the server reads the answer off the record that was chosen, so a crafted request can at worst pick the other person's real answer. A date's precision travels with the date (invariant 8) |
| `dismissDuplicate` | Records that two people are not the same, so the scan stops offering them. Lock-gated |
