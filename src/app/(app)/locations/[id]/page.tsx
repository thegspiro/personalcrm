...
export default async function LocationPage({
  ...
  const people = new Map<
    string,
    { id: string; name: string; visits: number; last: Date }
  >();
  for (const visit of location.interactions)
    for (const { contact } of visit.participants) {
    const current = people.get(contact.id);
      people.set(contact.id, {
        ...
        last:
          current?.last && current.last > visit.occurredAt
            ? current.last
            : visit.occurredAt,
      });
  }
    ...
    <div className="grid gap-5">
      <div>
        ...
        <div className="mt-2 flex items-start gap-3">
          <MapPin className="mt-1 size-5 text-accent-11" />
          <div>
            ...
            {location.isArchived ? (
              <p className="mt-1 text-xs text-muted-foreground">
                Archived — kept here, hidden from the list.
              </p>
            ) : null}
            <div className="mt-1 flex flex-wrap items-center gap-3">
              <a
                className="inline-flex items-center gap-1 text-xs text-accent-11 hover:underline"
                href={mapHref}
                target="_blank"
                rel="noreferrer"
              >
              ...
              <Link
                className="inline-flex items-center gap-1 text-xs text-accent-11 hover:underline"
                href={`/timeline?locationId=${location.id}&location=${encodeURIComponent(location.name)}`}
              >
                See in timeline
              </Link>
            </div>
      ...
      {location.phone || location.url || location.notes ? (
        <section className="rounded-lg border bg-card p-3">
          ...
          {location.url ? (
            <p className="text-sm">
              <a
                className="inline-flex items-center gap-1 text-accent-11 hover:underline"
                href={location.url}
                target="_blank"
                rel="noreferrer"
              >
      ...
      <section>
        ...
        {!people.size ? (
          <p className="text-sm text-muted-foreground">
            Nobody has been recorded here yet.
          </p>
        ) : null}
      </section>
      <section>
        <h3 className="mb-2 text-sm font-semibold">Visit history</h3>
        <div className="grid gap-2">
          {location.interactions.map((visit) => (
            <div key={visit.id} className="rounded-lg border bg-card p-3">
              <div className="flex justify-between gap-2">
                <p className="text-sm font-medium">
                  {visit.title ?? visit.type?.label ?? "Interaction"}
                </p>
                <time className="text-xs text-muted-foreground">
                  {date(visit.occurredAt)}
                </time>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">
                {visit.participants
            ...
            {location.plans.map((plan) => (
              <div key={plan.id} className="rounded-lg border bg-card p-3">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {plan.contact ? ` · with ${plan.contact.firstName}` : ""}
                </p>
              </div>
            ))}
          </div>
