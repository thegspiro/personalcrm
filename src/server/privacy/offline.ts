...
import { countPrivateRows } from "./counts";
 ...
 *     anyway.
 *  2. **There is nothing private to leak.** Nothing marked private anywhere in
 *     the account — the common case for someone who has never used the marker.
 *
 ...
 *
 * Memoised per request: the counts are asked once.
 */
export const offlineCacheable = cache(async (ownerId: string): Promise<boolean> => {
  ...
  return (await countPrivateRows(prisma, ownerId)) === 0;
});
